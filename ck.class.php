...
if(!class_exists('CkAndSyntaxHighlighter'))
{
	class CkAndSyntaxHighlighter
	{
		...
		{
			include_once($this->plugins_path ."/admin/index.php");
		}
		...
		function _create_ckeditor_script($textarea_id)
		{
			...
			function _load_ckeditor() {
				CK_editor = CKEDITOR.replace('<?php echo $textarea_id;?>',{
					...
					toolbar : 'MyToolBar',
					contentsCss : [<?php foreach($load_css as $key=>$temp):?>'<?php bloginfo('stylesheet_directory');?>/<?=$temp;?>'<?php if(($key+1) < count($load_css)):?>,<?php endif;?><?php endforeach;?>]
				});
				CKFinder.setupCKEditor(CK_editor, '<?php echo $this->plugins_url;?>ckfinder/');
			}
			_load_ckeditor();
			alert(document.getElementById('cke_contents_content').style);
			//]]>
			</script>
			<?php
		}
